...
4) Train the new team member the `Memory` skill as their first skill on day one. Instruct them to:
   ...
   - Read their `memory.md` at the start of every session before doing any other work.
   - Update their `memory.md` at the end of every session, when making key decisions, and when discovering important facts.
5) Train the new team member any other necessary skills they need to perform their tasks effectively, such as the `Read Email`, `Write Email`, `Archive Email` skills for communication, or any technical skills relevant to their role.
6) Communicate the new hire to the rest of the team and update any relevant project documentation to reflect the addition of the new team member. This may include updating project plans, task assignments, and team communication channels.
7) You can optionally ask the Morpheus agent to create a custom skill using the `Teach` skill if the agent requires a specific skill that is not already available in the project. This can help the new team member perform their tasks more efficiently and effectively.
